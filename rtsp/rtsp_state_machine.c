...
//        [RTSP_Method_PAUSE]    = RTSP_pause
    ...
    if ( cseq_val <= client->last_cseq ) {
        xlog(LOG_INF, "Duplicated CSeq: %"PRIu64". Dropping.",
                cseq_val);
...
//        xlog(LOG_INF, "Max connection reached");
//        if (twin) {
//            char *redir;
...
//            xlog(LOG_INF, "Redirecting to %s", redir);
    ...
    if ( rtsp->input->data[0] == '$' ) {
        ...
        default:
            ...
            case RFC822_Protocol_RTSP10:
                xlog(LOG_INF, "Incoming RTSP connection accepted");
                rtsp->status = RFC822_State_RTSP_Headers;
                break;

            case RFC822_Protocol_HTTP10:
            case RFC822_Protocol_HTTP11:
                xlog(LOG_INF, "Incoming HTTP connection accepted");
